...
# img = Image.effect_mandelbrot(size=(768, 512), extent=(-2, -1, 1, 1), quality=100)

def mandelbrot(width_px: int, height_px: int, location_re: np.float128, location_im: np.float128,
               zoom_level: np.float128, max_iterations: int) -> np.ndarray:
    ...
    color = np.full(shape=(width_px, height_px, 3), dtype=np.uint8, fill_value=[0, 255, 255])
    ...
    for screen_x in range(width_px):
        for screen_y in range(height_px):
            ...
            if z_re*z_re + z_im*z_im > 1:
                color[screen_x, screen_y, 0] = np.uint8((i + 1 - np.log2(abs(np.log2(z_re*z_re + z_im*z_im,
                                                                                     dtype=std_dtype))) *
                                                         255/max_iterations))
            else:
                color[screen_x, screen_y, 0] = np.uint8(i * 255 / max_iterations)
            if 10 >= (color[screen_x, screen_y, 0] % 245):
                color[screen_x, screen_y, 2] = color[screen_x, screen_y, 0] % 245
    return color
...
if __name__ == '__main__':
    ...
    start = time.time()
    img_asarray = mandelbrot(width_px=900, height_px=600, location_re=location_re, location_im=location_im,
                             zoom_level=100, max_iterations=400)
    print(f'single-threaded execution: {time.time() - start:.1f} seconds.')
